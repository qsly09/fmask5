...
class UNet(object):
    ...
    backbone = "resnet34"
    decoder_channels = [512, 256, 128, 64, 32]  # decorder_channels
    patch_size = 512
    patch_stride_train = 488  # for generating patches for training
    patch_stride_classify = 488  # for classifying the whole image
    batch_size = 16
    learn_rate = 1e-3
    epoch = 60  # 60 for Landsat 8 and Sentinel-2
    shuffle = True
    workers = 2
    prefetch = 2
    ...
    def train(self, path: str = None, save_epochs=None) -> None:
        """
        ...
        for epoch in range(epoch_start, self.epoch + 1):  # range starts at 1
            ...
            for data in train_dataset_loader:
                ...
                loss_train, overall_train = self.evaluate_model(
                    self.model, loss_fn, image_train, label_train, self.device
                )
                # Scale Gradients
            ...
            model_loss_test = np.nan  # preserve the nan value
    ...
    def classify(self, probability="none", normalize=True) -> tuple:
        ...
        if probability == "default":
            ...
            prob_index = self.classes.index(probability)
        ...
        rows_off = np.arange(
            0, datacube.shape[2] - self.patch_size + 1, self.patch_stride_classify
        )
        cols_off = np.arange(
            0, datacube.shape[3] - self.patch_size + 1, self.patch_stride_classify
        )
        rows_off = np.unique(np.append(rows_off, datacube.shape[2] - self.patch_size))
        cols_off = np.unique(np.append(cols_off, datacube.shape[3] - self.patch_size))

        # initlized image_class and image_prob
        image_class = np.full((datacube.shape[2], datacube.shape[3]), 0, dtype=np.uint8)
        if prob_index == -2:
            image_prob = None  # no need to initialize
        else:
            image_prob = np.full(
                (datacube.shape[2], datacube.shape[3]), 0, dtype=np.float32
            )
        self.model.eval()
        with torch.no_grad():  # disabled gradient calculation
            for r_off in rows_off:
                for c_off in cols_off:
                    # cut off the edge from the midpoint
                    if r_off == 0:
                        rows_image = [r_off, r_off + self.patch_size - wincut]
                        rows_patch = [0, self.patch_size - wincut]
                    elif r_off == rows_off[-1]:
                        rows_image = [
                            rows_off[-2] + self.patch_size - wincut,
                            image_class.shape[0],
                        ]
                        rows_patch = [
                            self.patch_size - rows_image[1] + rows_image[0],
                            self.patch_size,
                        ]
                    else:
                        rows_image = [r_off + wincut, r_off + self.patch_size - wincut]
                        rows_patch = [wincut, self.patch_size - wincut]

                    if c_off == 0:
                        cols_image = [c_off, c_off + self.patch_size - wincut]
                        cols_patch = [0, self.patch_size - wincut]
                    elif c_off == cols_off[-1]:
                        cols_image = [
                            cols_off[-2] + self.patch_size - wincut,
                            image_class.shape[1],
                        ]
                        cols_patch = [
                            self.patch_size - cols_image[1] + cols_image[0],
                            self.patch_size,
                        ]
                    else:
                        cols_image = [c_off + wincut, c_off + self.patch_size - wincut]
                        cols_patch = [wincut, self.patch_size - wincut]
                    ...
                    if (
                        ...
                        patchprob = patchprob[:, prob_index, :, :]
                        patchprob = patchprob.to("cpu").numpy()
                        image_prob[
                            rows_image[0] : rows_image[1], cols_image[0] : cols_image[1]
                        ] = patchprob[
                            0,
                            rows_patch[0] : rows_patch[1],
                            cols_patch[0] : cols_patch[1],
    ...
    @staticmethod
    def evaluate_model(model, loss_fn, image, label, device, overall=True):
    ...
    def __init__(
        ...
        Args:
            ...
            patch_size (int, optional): Size of patch in pixels. Defaults to 512.
            patch_stride_train (int, optional): Size of patch stride for trainning in pixels. Defaults to 488.
            patch_stride_classify (int, optional): Size of patch stride for classification in pixels. Defaults to 488.
