...
#%% test setups
update_rates = [0, 0.025, 0.05, 0.075, 0.1, 0.125, 0.15]

update_rates = [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1]
    ...
    for task in tasks:
       ...
       # check if the csv file exists ahead
        ...
        path_csv = os.path.join(
            destination,
            "lightgbm_cloud_physical",
            "update_rate", # "update_rate_optimal_seed_percentile",
            fmask0.image.name,
        ...
        if image_name_pre != os.path.basename(path_image):
            ...
            fmask.set_base_machine_learning("lightgbm")
        ...
        # Light GBM, set the seed levels for Sentinel-2
        if fmask.image.name[0] == 'S':
            ...
            if LANDSAT7: # for Landsat 7
                ...
                fmask.seed_levels = [25, 25]
        fmask.seed_levels = [0, 0] # should be set to 0, 0 for the physical model
        
        fmask.lightgbm_cloud.tune_update_rate = update_rate # setup the updating rate
        fmask.lightgbm_cloud.tune_append_rate = 0 # setup the appending rate
