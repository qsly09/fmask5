...
@click.option(
    ...
    default="/gpfs/sharedfs1/zhulab/Shi/ProjectCloudDetectionFmask5/Validation/Sentinel2ResultsCPU",
)
...
def main(resource, destination, ci, cn) -> None:
    ...
    for task in tasks:
        ...
        key = "unet"
        if os.path.exists(
            os.path.join(destination, image_name + f"_{key}.png")
        ):
        ...
        # get the colorfull image as well
        fmask.display_fmask(
            path=os.path.join(destination, fmask.image.name + f"_{key}.png")
        )

        # save accuracy for specific datasets
