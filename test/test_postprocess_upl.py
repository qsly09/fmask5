...
#%% test setups
dilations = [0, 20, 40, 60, 80, 100, 120, 140]
NUMBER = 10000
...
def main(resource, destination, ci, cn) -> None:
    ...
    for path_image in path_image_list:
        for dilation in dilations:
            tasks.append({"path_image": path_image, "dilation":dilation})
    ...
    for task in tasks:
       ...
       # check if the csv file exists ahead
        ...
        path_csv = os.path.join(
            ...
            "dilation", # "dilation",
            fmask0.image.name,
            f"{fmask0.image.name}_{int(dilation):04d}.csv"
        ...
        fmask.create_cloud_object(postprocess="unet", dilation = dilation, min_area=3)
