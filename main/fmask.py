...
Usage examples:
- python fmask.py --model=UPL --imagepath='/gpfs/sharedfs1/zhulab/Shi/ProjectCloudDetectionFmask5/HLSDataset/LC08_L1TP_048022_20230713_20230724_02_T1'
- python fmask.py --model=UPL --imagepath='/gpfs/sharedfs1/zhulab/Shi/ProjectCloudDetectionFmask5/HLSDataset/S2B_MSIL1C_20250117T183619_N0511_R027_T11SLT_20250117T221425.SAFE'
...
def fmask_physical(path_image, dcloud=0, dshadow=5, dsnow = 0, destination = None, skip = True, endname="PHY", metadata = False, display_fmask = False, display_image = False, print_summary = False):
    ...
    if skip and fmask.check_mask_existence(endname = endname):
        ...
        # clean the data to save memory
        fmask.image.clean_data()
        # save the mask
        fmask.save_mask(endname=endname)
        # save the model metadata
        if metadata:
            fmask.save_model_metadata(os.path.join(destination, fmask.image.name + f'_{endname}_meta.csv'), running_time=time.perf_counter() - time_start)
    ...
    if display_fmask:
        fmask.display_fmask(endname=endname,path = os.path.join(destination, fmask.image.name + f'_{endname}.png'), skip = skip)
    # msg
...
def fmask_unet(path_image, dcloud=0, dshadow=5, dsnow = 0, destination = None, skip = True, endname="UNT", metadata = False, display_fmask = False, display_image = False, print_summary = False):
    ...
    if skip and fmask.check_mask_existence(endname = endname):
        ...
        fmask.physical.init_constant_filter()
        fmask.mask_shadow(postprocess='none', min_area=0, potential = "flood") # not we do not exclude the very small cloud objects with less than 3 pixels if we use UNet results as output
        fmask.image.clean_data() # clean the data to save memory
        fmask.save_mask(endname=endname)
        if metadata:
            fmask.save_model_metadata(os.path.join(destination, fmask.image.name + f'_{endname}_meta.csv'), running_time=time.perf_counter() - time_start)
        if display_image:
...
def fmask_lpu(path_image, dcloud=0, dshadow=5, dsnow = 0, destination = None, skip = True, endname="LPU", metadata = False, display_fmask = False, display_image = False, print_summary = False):
    ...
    if skip and fmask.check_mask_existence(endname = endname):
        ...
        fmask.load_image()
        fmask.mask_cloud()
        fmask.mask_shadow(postprocess='none', min_area=0, potential = "flood") # no postprocessing for the cloud mask when unet is output
        fmask.image.clean_data() # clean the data to save memory
...
def fmask_upl(path_image, dcloud=0, dshadow=5, dsnow = 0, destination = None, skip = True, endname="UPL", metadata = False, display_fmask = False, display_image = False, print_summary = False):
    ...
    if skip and fmask.check_mask_existence(endname = endname):
        ...
        fmask.load_image()
        fmask.mask_cloud()
        fmask.mask_shadow(postprocess='unet', min_area=0, potential = "flood") # unet-based elimination for postpocessing
        fmask.image.clean_data() # clean the data to save memory
        fmask.save_mask(endname=endname)
        if metadata:
            fmask.save_model_metadata(os.path.join(destination, fmask.image.name + f'_{endname}_meta.csv'), running_time=time.perf_counter() - time_start)
...
@click.option(
    ...
    default="/gpfs/sharedfs1/zhulab/Shi/ProjectCloudDetectionFmask5/Validation/Landsat89/LC08_L1TP_006061_20210818_20210827_02_T1",
)
@click.option(
    "--output", "-o",
    type=str,
